...
Both workflows require the `IRIS_RELEASE_TOKEN` secret (a fine-grained PAT with `contents: write` + `issues: write`). This is the standard fix for `release: published` not triggering downstream workflows when the release is created by `GITHUB_TOKEN`. See `docs/secrets.md`.

**Version policy.** `package.json#version` is the single source of truth and follows semver. Routine work (refactors, chores, internal tooling) does NOT bump it — the release workflows own regular bumps and commit `chore(release): bump to X.Y.Z` directly to `main`. BUT when a PR ships something release-worthy (new user-facing feature, breaking change, or a UX overhaul like the Íris rebrand), agents must not forget the bump: do it in the same PR that lands the change, and say so in the PR body. Never bump while an RC window is open (the release branch owns the version during that window); queue it for the next cycle instead. If the app displays its version anywhere, keep those spots in sync with `package.json`.
