...
- **`dialog.tsx`**: `DialogContent` para de aplicar `bg-background`/`border`/`shadow-lg` cru e passa a envolver seu conteúdo em `Glass level={3}`. Como `Glass` não aceita `style`, a `DialogPrimitive.Content` (que já controla position/transform/animação via Radix) vira o wrapper posicionado, e `Glass` preenche por dentro — mesmo padrão já validado no menu de idioma do HUD (Fase 3, `HudSidebar.tsx`). O `X` do botão de fechar embutido (hoje `lucide-react`) vira `@phosphor-icons/react` (`XIcon`).
- **`dropdown-menu.tsx`**: `DropdownMenuContent`/`DropdownMenuSubContent` ganham `Glass level={3}` também — não `level={2}` como um rascunho anterior desta spec dizia: o precedente real da Fase 3 é o menu de idioma (um popover Radix portalizado pro `document.body`, exatamente a mesma categoria de `DropdownMenuContent`), que usou `level={3}`; `level={2}` na Fase 3 foi reservado a superfícies ancoradas junto de um painel persistente (a própria barra do HUD, seus avisos, os popups de dispositivo), categoria que não se aplica aqui. `Check`/`ChevronRight`/`Circle` (lucide) viram Phosphor (`CheckIcon`/`CaretRightIcon`/`CircleIcon`).
